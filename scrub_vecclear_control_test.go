...
// nothing else changed: same pin, same reserve-then-wipe, same defers. It is
// the control for the vector-clear proof — what the window's exit sequence
// leaves in the registers when nothing clears them — and it must stay in step
// with Scrub in scrub_legacy.go, minus the one line under test.
