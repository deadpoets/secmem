...
| The borrow, copy and compare paths clear the registers on return | With the same probe, both register files are planted inside the callback of `WithBytes`, `WithBytesErr` and `ArenaSlot.WithBytesErr`, and just before `CopyIn`, `CopyOut`, `ConstantTimeEqual`, `WriteTo`, `ReadFrom` and `NewBuffer`; after each returns no planted vector byte and no planted general-purpose word may remain. A control plants through a plain function call and must see both survive (all 240 vector bytes and ten registers on windows/amd64). Shown to fail with the clear removed from `WithBytes` and from `CopyIn`. End to end, `secmem-crypto`'s `WithBytesErr/copy-then-preempted` residue scenario found about forty fragments of a secret on the stack without the clear and none with it; it is skipped against a released core older than the one that carries the clear | `borrow_regclear_test.go`; `secmem-crypto/residue_scenarios_test.go` |
