...
| Legacy PEM encryption stays refused, and says so as a decision | Both entry points are given `Proc-Type` / `DEK-Info` files across the DES, 3DES and AES `DEK-Info` ciphers; each refusal must wrap `ErrRetiredAlgorithm` alongside its usual sentinel and must name the command that converts the file. The complementary test is what gives the marker meaning: everything refused as merely unimplemented — `chacha20-poly1305@openssh.com`, aes128/192, an unknown KDF, PKCS#8 PBES2 — must **not** wrap it, so the marker cannot decay into a synonym for `ErrUnsupportedKey` | `secmem-crypto/retired_test.go` |
