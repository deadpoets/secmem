...
- **What each key type leaves in memory is measured, not argued**
  (`test-residue`). The out-of-process residue scan is `//go:build !race`
  — the race detector's shadow reservations make a whole-address-space scan
  meaningless — so a dedicated job runs it on linux/amd64 and linux/arm64,
  once on a legacy build and once with `GOEXPERIMENT=runtimesecret`, and
  another (`test-residue-windows`) on windows/amd64, each against an empty
  allowlist.
...
| `Scrub` clears the general-purpose registers on the thread that ran `fn`, and the clear reaches what `fn` left | The test-only probe plants a distinct, non-canonical-address word in every general-purpose register the Go ABI lets a callee clobber — BX, CX, DX, SI, DI, R8–R13, R15 on amd64; R1–R17, R19–R27 on arm64 (the probe's pointer register is zeroed by the clear but cannot be observed) — inside a window and dumps them after it. Ordinary code uses these registers, so the proof reasons per register: back to back the probe must observe most of them; the window's exit sequence without the clear (the same pinned control as the vector proof) must let at least one planted word survive, and a none is a failure; after `Scrub`, `ScrubErr` and a panicking `Scrub` no register may hold its word. Without the clear the CI runners measured two of twelve surviving on linux/amd64, seven of twelve on windows/amd64, fourteen of twenty-six on linux/arm64 and nineteen of twenty-six on darwin/arm64; with it, none anywhere. Not under `-race`, whose instrumentation calls overwrite the registers between fill and dump; CI runs it in its own non-race step on Linux (both architectures, both build modes), macOS and Windows. Shown to fail with the general-purpose clear removed. The residue this closes was found by `secmem-crypto`'s residue test on linux/arm64: `memmove` moves a 17–32 byte copy through R6/R7 and R12/R13, and the next asynchronous preemption outside the window saved them to the stack | `scrub_gpclear_test.go`, `scrub_gpclear_amd64_test.go`, `scrub_gpclear_arm64_test.go` |
| The borrow, copy and compare paths clear the registers on return | With the same probe, both register files are planted inside the callback of `WithBytes`, `WithBytesErr` and `ArenaSlot.WithBytesErr`, and just before `CopyIn`, `CopyOut`, `ConstantTimeEqual`, `WriteTo`, `ReadFrom` and `NewBuffer`; after each returns no vector register may still hold its whole planted value and no general-purpose register its planted word — whole registers, because code running after a subject leaves its own values there and over hundreds of bytes some match a planted byte by chance (measured once on linux/arm64). A control plants through a plain function call and must see both survive (all fifteen observed vector registers and ten general-purpose ones on windows/amd64; all thirty-two and twenty-three on linux/arm64). Shown to fail with the clear removed from `WithBytes` and from `CopyIn`. End to end, `secmem-crypto`'s `WithBytesErr/copy-then-preempted` residue scenario found about forty fragments of a secret on the stack without the clear and none with it; it is skipped against a released core older than the one that carries the clear | `borrow_regclear_test.go`; `secmem-crypto/residue_scenarios_test.go` |
...
| The core's vector-register clear reaches what `secmem-crypto`'s windows leave | The fork's own clear is gone; these tests show the clear at the end of the `Scrub` window covers it. Argon2: the SSE blamka run bare must leave block state visible in X0–X15 (control; a zero is a failure, not a skip), the same step inside a worker-shaped `Scrub` window and a whole `Derive` must leave the file all zero. The passphrase path: the same bcrypt_pbkdf derivation and AES-CTR run bare over plain memory must leave residue (not `opensshCrypt` itself, whose scratch borrow clears the registers when it returns since core v0.6.0), and `opensshCrypt` run inside the `ScrubErr` window its callers use must leave none, shown to fail with the core's register clear removed. Pinned to one thread for the whole sequence; the dump is a test-only assembly probe (`secmem-crypto/internal/regprobe`) | `secmem-crypto/internal/argon2/scrubclear_amd64_test.go`, `secmem-crypto/vecclear_amd64_test.go` |
...
| What each entry point leaves in a live process's memory, per build mode | A victim subprocess receives known key material straight into a `SecureBuffer` (read from its stdin into the borrowed slice, never through a buffered reader), builds the object, uses it, is garbage-collected, and is destroyed; at each phase its parent freezes it and reads every readable page of it, counting hits outside the memory the kernel reports as locked — on Linux with `SIGSTOP`, `/proc/<pid>/mem` and `smaps`; on Windows by suspending every thread of the victim, reading it with `VirtualQueryEx` and `ReadProcessMemory`, and asking `QueryWorkingSetEx` per page whether the kernel has it locked, which a control pins by requiring a `SecureBuffer` page to be reported locked and a heap page not to be. It hunts every encoding that recovers the key, not just the raw bytes: big- and little-endian integers and the RSA FIPS-form key's limbs and Montgomery constants; the Ed25519 expanded secret, nonce digest, and both scalars in `edwards25519`'s internal layout; the HMAC key XORed into both pads and the SHA-256 chaining value after each pad block; the ML-KEM secret polynomial, σ, the SHAKE state that absorbed z and, as a separate scenario, the message each decapsulation recovers (encapsulated with a known message through `crypto/mlkem/mlkemtest`), and on the sender side the message, the shared key and the encryption randomness beside it (`Encapsulate`'s wrapper driven through the same derandomized encapsulation); the full AES round-key schedules; Argon2's H0; and every derived output. A *contained* entry point must leave nothing outside locked memory after construction, between operations, after GC, or after `Destroy`; a *transient* one must leave copies after use — so a standard-library change that removes them fails the test and forces the docs to be rewritten — and on a `runtimesecret` build they must be gone within 12 GC cycles. Four controls pin the premises: a heap copy made inside `Scrub` is erased by the collector only on a `runtimesecret` build, and one made outside it survives on both; and a secret copied between two locked buffers while another goroutine forces asynchronous preemption is saved onto the goroutine stack from the registers the copy used — on both builds — unless the copy runs inside `Scrub`, whose signal mask and register clears leave nothing. (On linux/arm64 that second control is what showed the general-purpose registers needed clearing too.) Where `Capabilities.AsyncPreemptSuppressed` is false — Windows, which has no preemption signal to block — that last control asserts the spill instead, and measures what the platform cannot prevent. Every scan must also find a heap canary the victim keeps alive, so a scan that reads nothing fails rather than passes. A scenario whose class depends on a core fix names the first core release that carries it, and is skipped with that reason when the module resolves an older released core (the `released-deps` job) rather than failing there. The result per key type is in [PROTECTION.md](PROTECTION.md). Shown to fail against a seed copied to the heap, against a copy of only the Ed25519 nonce scalar's internal layout, against a scanner that skips unlocked memory, against ECDSA reclassified as contained, and against runtime erasure claimed for a copy made outside `Scrub`, and on Windows against a scan that reads nothing and one that reports every page locked | `secmem-crypto/residue_test.go`, `residue_linux_test.go`, `residue_windows_test.go`, `residue_scenarios_test.go` |
| RSA, ECDSA and ML-KEM keys are refused where their heap copies are never erased | The shipped policy is pinned to `secmem.RuntimeSecretActive` by identity and by value, so a permissive default fails the suite. Under a refusing policy all six constructors return `ErrHeapTransients` before reading their input and without taking ownership of a buffer, the parsers refuse RSA and EC files through both entry points while Ed25519 files parse, `AllowHeapTransients` admits all of them, and the X25519 constructors and `Encapsulate`, which are not gated, succeed. Shown to fail against a permissive default, a parser that drops the option, and an ungated `NewMLKEM768Key`; run on a real `GOEXPERIMENT=runtimesecret` build as well as a legacy one | `policy_test.go`; the SSH agent example's `TestAdd_ECDSARefusedWithoutOptIn` |
...
- **The stack residue `Scrub` cannot reach is argued, not measured.** The frame
  ...
  Two residue sources remain unobservable from Go and are documented rather
  than tested: a GC stack-shrink that frees `fn`'s segment before the deferred
  wipe runs (`shrinkstack` is asynchronous and runtime-owned), and the
  `ucontext` the kernel writes to the signal stack on a synchronous fault. The
  first is closed by `GOEXPERIMENT=runtimesecret`, which has runtime
  cooperation; the second is a constraint of the OS. Both are enumerated in the
  stack-residue section of [THREAT-MODEL.md](THREAT-MODEL.md). (The
  general-purpose registers at `Scrub`'s return used to be listed here as
  unverifiable; they are now cleared and proven, see the table.)
