...
func TestDERWriter_Lengths(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{derInt{big: new(big.Int)}, []byte{0x02, 0x01, 0x00}},
		{derInt{big: big.NewInt(0x7f)}, []byte{0x02, 0x01, 0x7f}},
		{derInt{big: big.NewInt(0x80)}, []byte{0x02, 0x02, 0x00, 0x80}},
		{derInt{big: big.NewInt(0x100)}, []byte{0x02, 0x02, 0x01, 0x00}},
	} {
		buf := make([]byte, 8)
		w := derWriter{b: buf}
		w.putInteger(tc.x)
